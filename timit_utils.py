...
def list_inception(mylist):
    if isinstance(mylist, int) or isinstance(mylist, str):
        return 0 #int or str
    elif isinstance(mylist[0], int) or isinstance(mylist[0], str):
        return 1 #list of int or str
    elif isinstance(mylist[0][0], int) or isinstance(mylist[0][0], str):
...
def remove_q(y):
    ...
    if deepness == 0:
        raise Exception("Should provide something deeper, a list at least")
    elif deepness == 1:
...
def labels_to_str(y, reduced = True):
    ...
    def _labels_to_str(labels):
        return np.array([phons_redu_keys[y] for y in labels])
