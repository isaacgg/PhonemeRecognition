...
#        dropout_cte = tf.nn.dropout(tf.ones_like(hidden_p), self.dropout_rate)###dropout
#        hidden_p = tf.multiply(hidden_p, dropout_cte)###dropout
...
#        hidden_recon_act = self.sample_sigmoid(hidden_recon_mf, hidden_recon_p)
        ...
        negative_grad = tf.matmul(tf.transpose(visible_recon_act), hidden_recon_p)/tf.to_float(tf.shape(self.x)[0])
...
#            self.update_weights += [update_sigma]
        ...
        self.compute_visible = tf.nn.relu(tf.matmul(self.compute_hidden, tf.transpose(self.w)) + self.visible_bias)
        self.compute_visible_from_hidden = tf.nn.relu(tf.matmul(self.y, tf.transpose(self.w)) + self.visible_bias)
