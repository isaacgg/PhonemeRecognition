...
from Floyd.scripts.commons.TF_base import TF_base

class RNN_CTC_base(TF_base):    
    """ PARAMETRIZABLES """
    def rnn_cell(self):
    ...
    def build_ctc(self):
        with self._graph.as_default():            
            ...
            with tf.name_scope("variables"):
                w = tf.get_variable(name = "W", shape = [rnn_out.get_shape()[2], self.n_classes],
                                    dtype = tf.float32, 
            ...
            with tf.name_scope("calc_accuracies"):
                self.accuracy = tf.constant(1.0) - tf.reduce_mean(tf.edit_distance(tf.to_int32(self.decoded), sparse_y))
